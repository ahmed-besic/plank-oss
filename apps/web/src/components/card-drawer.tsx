...
function GenericCardDrawer({
  ...
  const finishRenameTag = (tagId: string) => {
    const name = editingTagName.trim()
    if (!name || updateTag.isPending) {
      setEditingTagId(null)
      return
    }
    updateTag.mutate({ tagId, name })
    setEditingTagId(null)
  }
  ...
  useEffect(() => {
    ...
    const onPointerDown = (event: PointerEvent) => {
      ...
      if (tagColorPickerRef.current?.contains(event.target as Node)) {
        return
      }
      if (
        tagColorPickerId &&
  ...
  }, [activePopover, tagColorPickerId])
  ...
  return (
    <>
                    ...
                    {activePopover === 'tags' ? (
                      <div className="absolute left-0 top-full z-20 mt-2 w-64 rounded-xl border border-border-subtle bg-cloud-white p-1.5 shadow-2xl">
                        ...
                        <div className="flex items-center gap-2 rounded-lg px-2 py-1.5">
                          <div className="relative shrink-0">
                            <button
                              aria-label="Choose new tag color"
                              className="flex h-7 w-7 items-center justify-center rounded-md transition hover:bg-cloud-white"
                              ref={(element) => {
                                tagColorTriggerRefs.current.new = element
                              }}
                              onClick={() =>
                                setTagColorPickerId(
                                  tagColorPickerId === 'new' ? null : 'new',
                                )
                              }
                              type="button"
                            >
                              <span
                                className="h-3 w-3 rounded-full"
                                style={getTagDotStyle(newTagColor)}
                              />
                            </button>
                          </div>
                          <input
      ...
      />
      {activePopover === 'tags' && tagColorPickerId && tagColorPickerRect
        ? createPortal(
            <div
              ref={tagColorPickerRef}
              className="fixed z-[70] flex max-h-48 w-32 flex-col gap-1 overflow-y-auto rounded-xl border border-border-subtle bg-cloud-white p-1.5 shadow-xl"
              style={{
                left: tagColorPickerRect.right + 8,
                top: tagColorPickerRect.top,
              }}
            >
              {TAG_COLOR_PALETTE.map((option) => (
                <button
                  key={option.key}
                  className="flex w-full items-center gap-1.5 rounded-lg px-2 py-1.5 text-left text-xs text-text-secondary transition hover:bg-surface-sunken hover:text-text-primary"
                  onClick={(event) => {
                    event.stopPropagation()
                    if (tagColorPickerId === 'new') {
                      setNewTagColor(option.key)
                    } else {
                      updateTag.mutate({
                        tagId: tagColorPickerId,
                        color: option.key,
                      })
                    }
                    setTagColorPickerId(null)
