import { COMMENT_REACTION_KEYS, type CommentReactionKey, type MentionRange } from "@plank/domain";
...
export const create = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const mentions = await filterWorkspaceMentions({
      ctx,
      workspaceId: workspace._id,
      mentions: normalizeCommentMentions(bodyText, args.mentions as MentionRange[]),
    });
    const now = Date.now();
